...
class Module_Authorization_Hook implements Interface_Hook{
   ...
   public static function init(){
      ...
      add_filter(PREFIX . '\dropsync\initiate', [__CLASS__, 'is_access_granted']);
      add_filter(PREFIX . '\dropsync_shipping\initiate', [__CLASS__, 'is_access_granted']);

      add_filter(PREFIX . '\synchronization\initiate', [__CLASS__, 'is_access_granted']);
   ...
   public static function add_in_log_criteria_list($items){
      ...
      $items['not_authorized'] = [
         'type'    => 'warning',
         'message' => sprintf(__('You have to authorize the plugin. Please go to %sthis page%s.', 'integration-adyen-woocommerce'), '<a href="'.SETTINGS_URL.'&section=authorization">', '</a>'),
         'hook'    => 'admin_init',
