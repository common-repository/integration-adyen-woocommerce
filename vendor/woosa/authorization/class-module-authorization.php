...
class Module_Authorization{
   ...
   public function get_status(){
      ...
      $value = __('Unauthorized', 'integration-adyen-woocommerce');

      if( $this->is_authorized() ){
         $value = __('Authorized', 'integration-adyen-woocommerce');
      }
   ...
   public static function render_status($environment = ''){
      ...
      $status = '<b>'.__('Status:', 'integration-adyen-woocommerce').'</b> <span style="color: '.$color.';">'.$ma->get_status().'</span>';

      $btn_attr = "data-" . PREFIX . "-authorization='{$data}'";
      $btn_label = $ma->is_authorized() ? __( 'click to revoke', 'integration-adyen-woocommerce' ) : __( 'click to authorize', 'integration-adyen-woocommerce' );
   ...
   public function get_wiki_article_url() {
      ...
      switch ($this->get_service()) {
         ...
         case 'vidaxl':
            $wiki_article_url = 'https://www.woosa.com/help/docs/how-to-authorise-vidaxl-in-your-plugin/';
            break;

         case 'big-buy':
            $wiki_article_url = 'https://www.woosa.com/help/docs/how-to-authorise-bigbuy-in-the-plugin-settings/';
            break;

         case 'vandermeer':
            $wiki_article_url = 'https://www.woosa.com/help/docs/how-to-authorise-van-der-meer-in-the-plugin-settings/';
            break;

         case 'kaufland':
            $wiki_article_url = 'https://www.woosa.com/help/docs/how-to-authorise-kaufland-in-the-plugin-settings/';
            break;

         case 'loyverse':
            $wiki_article_url = 'https://www.woosa.com/help/docs/authorise-loyverse/';
            break;
