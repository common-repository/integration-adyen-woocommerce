   ...
   public function display_product_status(){
      ...
      if ( $this->is_product_type( 'variable' ) ) {
         ...
         if(in_array($meta->get_status(), ['error', 'in_progress'])){
            ...
            $variation_ids = array_column($this->get_product()->get_available_variations(), 'variation_id');
            ...
            if(empty($status_list)){
               ...
               foreach( $status_list as $status => $status_total ) {
                  ...
                  $count = sprintf( _n( '%s variation', '%s variations', $status_total, 'woosa-loyverse' ), $status_total );
