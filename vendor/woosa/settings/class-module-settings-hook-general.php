...
class Module_Settings_Hook_General implements Interface_Hook_Settings{
   ...
   public static function section_name(){
      return __('Settings', 'integration-adyen-woocommerce');
   }
   ...
   public static function init(){
      ...
      add_action(PREFIX . '\field_generator\render\\' . PREFIX .'_field_value_source', [__CLASS__, 'output_field_value_source'], 99);

      add_action('woocommerce_admin_field_' . PREFIX .'_use_wc_price', [__CLASS__, 'output_use_wc_price'], 99);
      add_action(PREFIX . '\field_generator\render\\' . PREFIX .'_use_wc_price', [__CLASS__, 'output_use_wc_price'], 99);
   ...
   public static function add_section_fields($items){
      ...
      $items[] = [
         'name' => __('Misc', 'integration-adyen-woocommerce'),
      ...
      if(defined(__NAMESPACE__ . '\PRESERVE_STOCK_OFFSET') && PRESERVE_STOCK_OFFSET){
         $items[] = [
            'name'     => __('Preserve Stock Offset', 'integration-adyen-woocommerce'),
            'id'       => PREFIX .'_preserve_stock_offset',
            'desc' => sprintf(
               __('Define a value that will be subtracted from the product stock. This will help to avoid selling out of stock products. For more details %scheck this page%s.', 'woosa-bigbuy'),
               '<a href="https://www.woosa.com/help/docs/preserve-stock-offset/" target="_blank">',
               '</a>'
            ),
            'type'     => 'number',
            'default'  => '0',
            'validation' => [
               'message'     => __('"Preserve Stock Offset" must be greater than or equal with 0.', 'integration-adyen-woocommerce'),
               'value'       => [
                  [
                     'compare'  => 0,
                     'operator' => '>='
      ...
      $items[] = [
         'name' => __('Debug Mode', 'integration-adyen-woocommerce'),
         'id'   => PREFIX .'_debug',
         'type' => 'checkbox',
         'desc' => __('Enable', 'integration-adyen-woocommerce'),
         'desc_tip' => __('Set whether or not to enable debug mode.', 'integration-adyen-woocommerce'),
         'default' => 'no',
         'autoload' => false,
      ];

      $items[] = [
         'name' => __('Remove Configuration', 'integration-adyen-woocommerce'),
         'id'   => PREFIX .'_remove_config',
         'type' => 'checkbox',
         'desc' => __('Yes', 'integration-adyen-woocommerce'),
         'desc_tip' => __('Set whether or not to remove the plugin configuration on uninstall.', 'integration-adyen-woocommerce'),
         'default' => 'no',
         'autoload' => false,
      ];
   ...
   public static function add_action_link($links){
      ...
      $links['actions'] = array_merge([
         SETTINGS_URL => __('Settings', 'integration-adyen-woocommerce')
      ], $links['actions']);
